...
const generateToken = (userId, expires, type, secret = config.jwt.secret) => {
  const token = jwt.sign({sub : userId , exp:expires, type : type},secret);
...
const generateAuthTokens = async (user) => {
  const token = generateToken(user._id,Date.now()/1000+config.jwt.accessExpirationMinutes*60,tokenTypes.ACCESS,config.jwt.secret);
  return {
    access : {
      token,
